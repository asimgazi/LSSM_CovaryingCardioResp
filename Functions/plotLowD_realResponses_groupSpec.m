function plotLowD_realResponses_groupSpec(dim2plot, plotTitle, fileName, ...
    featSegm_group, Fs, varargin)
...
% be a vector of subject numbers to use for legends.
% What this flag entails is it will set
...
% 'BSN2023' - flag if you want to create plots specifically for BSN 2023
%             paper (ignores stimulation conditions)
...
for arg = 1:length(varargin)
    ...
    elseif strcmp(varargin{arg}, 'BSN2023')
        BSN2023 = true;
    end
end
...
numSubjects = numel(featSegm_group);  % # subjects can differ between groups
...
for cond_idx = 1:numConds
    numSamples(cond_idx) = ...
        size(featSegm_group{1}.(condNames{cond_idx}), 1);
end
numFeats = size(featSegm_group{1}.(condNames{1}), 2);
...
if condAvg
    ...
    % Separate responses by condition; within each separation,
    % stack across all subjects
    resps_condSep = cell(numConds, 1);
    for cond_idx = 1:numConds
        % Initialize
        resps_condSep{cond_idx} = ...
            zeros(numSamples(cond_idx), numFeats, numSubjects);
    end
    
    % Now fill the cell array's arrays
    for cond_idx = 1:numConds
        for sub = 1:numSubjects
            resps_condSep{cond_idx}(:, :, sub) = ...
                feat_condAvg_group{sub}.(condNames{cond_idx});
        end
    end
    ...
    if meanResp
        ...
        meanResps_condAvg = cell(numConds, 1);
        for cond_idx = 1:numConds
            meanResps_condAvg{cond_idx} = ...
                mean(resps_condSep{cond_idx}, 3);
        end
        ...
        for cond_idx = 1:numConds
            ...
            condLabels = [condLabels; cond_idx*...
                ones(size(meanResps_condAvg{cond_idx}, 1), 1)];
        end
        
        % Check to make sure sizes are correct
        if size(stackedResps, 1) ~= sum(numSamples) || ...
                length(condLabels) ~= sum(numSamples)
        ...
        % Separate the PCA scores by condition to make labeling of
        ...
        for cond_idx = 1:numConds
            ...
            if size(plotReady{cond_idx}, 1) ~= numSamples(cond_idx) || ...
        ...
        if dim2plot == 2
            % Create video
            %(Note that I transpose plotReady because video function wants
            % plotReady to be a (# markers)x(#colors) cell array
            video_lowDtrajectories(plotReady', timeVec, dim2plot, ...
                fileName, 'plotTitle', plotTitle, ...
                'clrNames', condNames, ...
                'xlabel', ...
            ...
            video_lowDtrajectories(plotReady', timeVec, dim2plot, ...
                ...
                ['PC2 Var. Expl. : ', num2str(varExplain(2)), '%'], ...
                'zlabel', ...
                ['PC3 Var. Expl. : ', num2str(varExplain(3)), '%'], ...
                'velocities');
        ...
        % Create cell array of subject ID strings
        subjectIDs = cell(numSubjects, 1);
        for sub = 1:numel(subjectIDs)
            subjectIDs{sub} = num2str(subjectNums(sub));
        end
        
        % Since I am not doing any averaging across subjects, I essentially
        % need to stack and flatten all of the
        % responses into a 2-D array of dimension:
        % numSubjects*sum(numSamples) x numFeats
        % At the same time, I will need to keep track of
        ...
        % Check to make sure sizes are correct
        if size(stackedResps, 1) ~= numSubjects*sum(numSamples) || ...
                length(subjLabels) ~=  numSubjects*sum(numSamples) || ...
                length(condLabels) ~= numSubjects*sum(numSamples)
            error('Stacking did not go as planned');
        end
        ...
        % Separate the PCA scores by condition and subject to make
        % labeling of plots easy (i.e., "unstack")
        plotReady = cell(numConds, 1);
        
        for cond_idx = 1:numConds
            plotReady{cond_idx} = cell(numSubjects, 1);
            for sub_idx = 1:numSubjects
                ...
                % Check size to make sure everything was stored properly
                if size(plotReady{cond_idx}{sub_idx}, 1) ~= ...
                        numSamples(cond_idx) || ...
                        size(plotReady{cond_idx}{sub_idx}, 2) ...
                        ~= numFeats
                    error('plotReady sizing is incorrect')
                end
            end
    ...
    % Separate responses by condition type; within each
    ...
    resps_condSep_repSep = cell(numConds, 1);
    for cond_idx = 1:numConds
        % Initialize
        resps_condSep_repSep{cond_idx} = ...
            cell(conds_perType(cond_idx), 1);
        for rep_idx = 1:conds_perType(cond_idx)
            % Initialize
            resps_condSep_repSep{cond_idx}{rep_idx} = ...
                zeros(numSamples(cond_idx), numFeats, numSubjects);
        end
    end
    ...
    for cond_idx = 1:numConds
        for rep_idx = 1:conds_perType(cond_idx)
            for sub = 1:numSubjects
                resps_condSep_repSep{cond_idx}{rep_idx}(:, :, sub) = ...
                    featSegm_group{sub}.(condNames{cond_idx})(:, :, rep_idx);
            end
        end
    end
    ...
    if meanResp
        ...
        % Compute means for each condition repetition
        ...
        for cond_idx = 1:numConds
            ...
            for rep_idx = 1:conds_perType(cond_idx)
                meanResps_condAvg{cond_idx}{rep_idx} = ...
                    mean(...
                    resps_condSep_repSep{cond_idx}{rep_idx}, ...
                    3, 'omitnan');
            end
            ...
            for cond_idx = 1:numConds
                for rep_idx = 1:conds_perType(cond_idx)
                    ...
                    repLabels = [repLabels; rep_idx*...
                        ones(size(meanResps_condAvg{cond_idx}{rep_idx}, 1), 1)];
                    condLabels = [condLabels; cond_idx*...
                        ones(size(meanResps_condAvg{cond_idx}{rep_idx}, 1), 1)];
                end
            end
        else
            % For BSN 2023, we ignored stim. conditions, so we start at 2
            for cond_idx = 2:numConds
                for rep_idx = 1:conds_perType(cond_idx)
                    % Stack responses
                    stackedResps = [stackedResps; ...
                        meanResps_condAvg{cond_idx}{rep_idx}];
        ...
        % Now check size
        if size(stackedResps, 1) ~= totalSamples || ...
                length(repLabels) ~= totalSamples || ...
                length(condLabels) ~= totalSamples
            error('Stacking did not go as planned');
        end
        ...
        % Separate the PCA scores by condition and repetition to
        % make labeling of plots easy (i.e., "unstack")
        plotReady = cell(numConds, 1);
        
        for cond_idx = 1:numConds
            % Nested cell arrays to store the repetitions per condition
            plotReady{cond_idx} = cell(conds_perType(cond_idx), 1);
            for rep_idx = 1:conds_perType(cond_idx)
                plotReady{cond_idx}{rep_idx} = ...
                    pcaResult(repLabels == rep_idx & ...
                    condLabels == cond_idx, :);
                
                % Check size to make sure everything was stored properly
                if ~BSN2023
                    if size(plotReady{cond_idx}{rep_idx}, 1) ~= ...
                            numSamples(cond_idx) || ...
                            size(plotReady{cond_idx}{rep_idx}, 2) ...
                            ~= numFeats
                        error('plotReady sizing is incorrect')
                    end
                else
                    % Ignore the check for condition 1 (i.e., stimulation)
                    if (size(plotReady{cond_idx}{rep_idx}, 1) ~= ...
                            numSamples(cond_idx) && cond_idx > 1)|| ...
                            size(plotReady{cond_idx}{rep_idx}, 2) ...
                            ~= numFeats
                        error('plotReady sizing is incorrect')
                    end
        ...
        if dim2plot == 2
            % To create video, I will need to reformat things to match the
            % video function
            %(video function wants plotReady to be (# markers)x(#colors))
            
            % First, create cell array of strings for color names (legend)
            legendNames = cell(sum(conds_perType), 1);
            for cond_idx = 1:numConds
                for rep_idx = 1:conds_perType(cond_idx)
                    legendNames{sum(conds_perType(1:(cond_idx - 1)))...
                        + rep_idx} = [condNames{cond_idx}, ...
                        num2str(rep_idx)];
                end
            end
            
            % Restructure plotReady to be cell(1, sum(conds_perType))
            % That way, each repetition will be a separate color
            plotReady_interim = plotReady;
            plotReady = cell(1, sum(conds_perType));
            for cond_idx = 1:numConds
                for rep_idx = 1:conds_perType(cond_idx)
                    % Reformat just like I did for the legend names
                    plotReady{sum(conds_perType(1:(cond_idx - 1)))...
                        + rep_idx} = plotReady_interim{cond_idx}{rep_idx};
                end
            end
        ...
        % each subplot will have all the subjects' data overlaid. What we
        ...
        for cond_idx = 1:numConds
            for rep_idx = 1:conds_perType(cond_idx)
                for sub_idx = 1:numSubjects
                    ...
                    if ~isnan(sum(resps_condSep_repSep{cond_idx}{...
                            rep_idx}(:, :, sub_idx), 'all'))
                        stackedResps = [stackedResps; ...
                            resps_condSep_repSep{cond_idx}{...
                            rep_idx}(:, :, sub_idx)];
        ...
        for cond_idx = 1:numConds
            ...
            for rep_idx = 1:conds_perType(cond_idx)
                ...
                plotReady{cond_idx}{rep_idx} = cell(numSubjects, 1);
                for sub_idx = 1:numSubjects
                    % If data does not exist, then the cell will remain
                    % empty, and I can find this using isempty()
                    plotReady{cond_idx}{rep_idx}{sub_idx} = ...
                        pcaResult(subjLabels == subjectNums(sub_idx) & ...
                        condLabels == cond_idx & repLabels == rep_idx, :);
                    
                    % Check size to make sure everything was stored properly
                    if ~isempty(plotReady{cond_idx}{rep_idx}{sub_idx}) ...
                            && (size(plotReady{cond_idx}{...
                            rep_idx}{sub_idx}, 1) ~= numSamples(cond_idx) || ...
                            size(plotReady{cond_idx}{rep_idx}{sub_idx}, 2) ...
                            ~= numFeats)
                        error('plotReady sizing is incorrect')
                    end
                end
        ...
        if dim2plot == 2
            ...
            for cond_idx = 1:numConds
                ...
                numRows = ceil(conds_perType(cond_idx)/subs_perRow);
                subs = tight_subplot(numRows, subs_perRow);
                ...
                for rep_idx = 1:conds_perType(cond_idx)
                    ...
                    for sub_idx = 1:numSubjects
                        % Plot one scatter point at a time to leverage time labels
                        if ~isempty(plotReady{cond_idx}{...
                                rep_idx}{sub_idx})
                            % Add this subject ID to legend
                            legendNames = [legendNames; ...
                                subjectIDs{sub_idx}];
                            for t = 1:size(plotReady{cond_idx}{...
                                    rep_idx}{sub_idx}, 1)
                                s = scatter(plotReady{cond_idx}{...
                                    rep_idx}{sub_idx}(t, 1), ...
                                    plotReady{cond_idx}{...
                                    rep_idx}{sub_idx}(t, 2), ...
                                    sizeLabels(t), colorLabels(sub_idx, :), ...
                    ...
                    % Title it according to condition
                    % repetition
                    title([condNames{cond_idx},'-', num2str(rep_idx)]);
                    hold off
                ...
                % Add title to entire figure
                sgtitle([plotTitle, ' - Condition Type: ', ...
                    condNames{cond_idx}]);
                
                % Save plot
                saveas(fig, [fileName, ' - Condition Type_', ...
