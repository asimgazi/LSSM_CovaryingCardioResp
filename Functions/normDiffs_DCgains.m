function normDiffs_DCgains(plotTitle, fileName, aParams, Cparams, inputsOpt, ...
    varargin)
...
% normDiffs_CBproducts function, so many of the variable names and comments
% may not match up
...
if iscell(inputs)
    ...
    if ~isempty(norms_all)
        for sub = 1:numel(inputs)
            ...
            for p1 = 1:(numInputs-1)
                for p2 = (p1+1):numInputs
                    ...
                    norms_all(sub, p) = norm(Cparams{sub}*inv(eye(size(aParams{sub}, 1)) - ...
                        aParams{sub})*inputs{sub}(:, p1) - ...
                        Cparams{sub}*inv(eye(size(aParams{sub}, 1)) - ...
                        aParams{sub})*inputs{sub}(:, p2));
                    norms_div_all(sub, p) = ...
                        norm(Cparams{sub}*inv(eye(size(aParams{sub}, 1)) - ...
                        aParams{sub})*inputs{sub}(:, p1) - ...
                        Cparams{sub}*inv(eye(size(aParams{sub}, 1)) - ...
                        aParams{sub})*inputs{sub}(:, p2))/...
                        (norm(Cparams{sub}*inv(eye(size(aParams{sub}, 1)) - ...
        ...
        if numInputs == 2
            ...
            xticklabels({'DCgain_v - DCgain_t', 'DCgain_v - DCgain_n', ...
                'DCgain_t - DCgain_n'});
        end
        ...
        if numInputs == 2
            if strcmp(inputsOpt, 'vn')
                xticklabels({'(DCgain_v - DCgain_n)/norms of each'});
            else
                xticklabels({'(DCgain_v - DCgain_t)/norms of each'});
            end
        else
            xticklabels({'(DCgain_v - DCgain_t)/norms of each', ...
                '(DCgain_v - DCgain_n)/norms of each', ...
                '(DCgain_t - DCgain_n)/norms of each'});
        end
        ...
        if numInputs == 2
            if strcmp(inputsOpt, 'vn')
                xticklabels({'unit(DCgain_v) - unit(DCgain_n)'});
            else
                xticklabels({'unit(DCgain_v) - unit(DCgain_t)'});
            end
        else
            xticklabels({'unit(DCgain_v) - unit(DCgain_t)', ...
                'unit(DCgain_v) - unit(DCgain_n)', ...
                'unit(DCgain_t) - unit(DCgain_n)'});
        end
    ...
    if ~isempty(norms)
        ...
        for p1 = 1:(numInputs-1)
            for p2 = (p1+1):numInputs
                ...
                norms(p) = ...
                    norm(Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p1)...
                    - Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p2));
                norms_div(p) = ...
                    norm(Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p1)...
                    - Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p2))/...
                    (norm(Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p1))...
                    *norm(Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p2)));
                norms_unit(p) = norm(...
                    (Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p1)/...
                    norm(Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p1))) - ...
                    (Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p2)/...
                    norm(Cparams*inv(eye(size(aParams, 1)) - aParams)*inputs(:, p2)))...
                    );
