function angle_CBproducts(plotTitle, fileName, Cparams, inputsOpt, varargin)
...
if iscell(inputs)
    ...
    if ~isempty(angles_all)
        ...
        if numInputs == 2
            if strcmp(inputsOpt, 'vn')
                xticklabels({'C*b_v \cdot C*b_n'});
            else
                xticklabels({'C*b_v \cdot C*b_t'});
            end
        else
            xticklabels({'C*b_v \cdot C*b_t', 'C*b_v \cdot C*b_n', ...
                'C*b_t \cdot C*b_n'});
        end
