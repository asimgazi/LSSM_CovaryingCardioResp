function PCrecipe_constcy_realData(plotTitle, fileName, responses_cellSep, ...
    varargin)
% This function will take a set of real responses, apply PCA to each
...
%                    Each struct will have C fields, where C represents the
%                    number of conditions (e.g., C = 3 for VNS, trauma, and
%                    neutral). Each field will contain a T_i x M x R_i array.
%                    T_i represents the number of timepoints for the ith
%                    condition (i \in {1, ..., C}), M is the number of
%                    features, and R_i is the number of repetitions of the
%                    ith condition

% (optional)
% 'verbose' - if you want to see all warnings (default is not verbose)

% 'condSep' - flag that specifies that each repetition is treated separately

% 'condAvg' - flag that specifiees that repetitions of the same condition
%             are averaged

% 'condAvg_train' - if this flag is provided, then the next
%               input should be the train-test split condition code
%               (e.g., 6.5 for split between 6 and 7) so that only training
%               repetitions are used for averaging
...
for arg = 1:length(varargin)
    ...
    elseif strcmp(varargin{arg}, 'condSep')
        condAvg = false;
        trainOnly = false;
...
if ~exist('condAvg', 'var'); condAvg = true; end
if ~exist('trainOnly', 'var'); trainOnly = true; end
if ~exist('trTest_split', 'var'); trTest_split = 6.5; end
...
for c = 1:numCells
    if numSubcells(c) > 1
        ...
        if condAvg
            % We need to average over repetitions of same condition for
            % each subject
            
            % Initialize data structure
            condAvg_subjSep = cell(numSubcells(c), 1);
            
            if trainOnly
                % We should only consider training data
                ...
                for subcell = 1:numSubcells(c)
                    for cond = 1:numConds
                        % Average along the 3rd dimension to average over
                        % reptitions, but only include training data here
                        condAvg_subjSep{subcell}.(condNames{cond}) = ...
                            mean(responses_cellSep{c}{subcell}.(condNames{...
                            cond})(:, :, 1:trainNums(cond)), 3, 'omitnan');
                    end
                end
            else
                % We should consider all data
                for subcell = 1:numSubcells(c)
                    for cond = 1:numConds
                        % Average along the 3rd dimension to average over
                        % reptitions
            ...
            % We need to treat each condition's repetitions separately
            
            % What I want to do is essentially forego the organization
            % based on conditions and just create another layer of nested
            % cell arrays to just store all the repetitions without
            % worrying about what condition they are a part of. That way, I
            % can stack all the repetitions across subjects in the
            % corresponding cell and then if I need to take the mean, I cna
            % do that and if I don't, I can just unstack into a long array
            
            % I'll need to know how many repetitions per condition to make
            % looping easy
            reps_perCond = zeros(numConds, 1);
            for cond_idx = 1:numConds
                % Use first subcell arbitrarily
                reps_perCond(cond_idx) = ...
                    size(responses_cellSep{c}{1}.(condNames{cond_idx}), 3);
            end
            
            % Now, I can reorganize into nested cell array
            responses_condSep = cell(sum(reps_perCond), 1);
            cntr = 1; % Set counter
            for cond_idx = 1:numConds
                for rep = 1:reps_perCond(cond_idx)
                    for subcell = 1:numSubcells(c)
                        % Concatenate subjects' data in 3rd dimension
                        responses_condSep{cntr} = cat(3, ...
                            responses_condSep{cntr}, ...
                            responses_cellSep{c}{subcell}.(condNames{...
                            cond_idx})(:, :, rep));
                    end
            ...
            % At this point, I can now branch based on whether I need to
            % take the mean along the 3rd dimension for each of the cells
            % or not
            % Initialize a new cell array to store intermediate result after
            % next step
            cellSpec_resps_repReady = cell(sum(reps_perCond), 1);
            if meanResp
                % In this case, we need to compute the mean condition-specific
                % responses and then concatenate
                for rep = 1:sum(reps_perCond)
                    cellSpec_resps_repReady{rep} = ...
                        mean(responses_condSep{rep}, 3, 'omitnan');
                end
            else
                % In this case, we need to unstack along 3rd dimension and
                % instead concatenate along 1st dimension
                for rep = 1:sum(reps_perCond)
                    for indx = 1:size(responses_condSep{rep}, 3)
                        % I do need to check for NaNs though before stacking
                        if ~isnan(sum(responses_condSep{rep}(:, :, indx), ...
                                'all'))
                            cellSpec_resps_repReady{rep} = ...
                                [cellSpec_resps_repReady{rep}; ...
                                responses_condSep{rep}(:, :, indx)];
                        end
                    end
                end
            end
            
            % We can now stack across and store away
            for rep = 1:sum(reps_perCond)
                stackedArrays{c} = [stackedArrays{c}; ...
                    cellSpec_resps_repReady{rep}];
            end
        ...
        if condAvg
            ...
            if trainOnly
                ...
                for cond = 1:numConds
                    % Average along the 3rd dimension to average over
                    % reptitions, but only include training data here
                    responses_condAvg.(condNames{cond}) = ...
                        mean(responses_cellSep{c}.(condNames{...
                        cond})(:, :, 1:trainNums(cond)), 3, 'omitnan');
                end
            ...
            % We need to treat each condition's repetitions separately, but
            % I don't need to worry about stacking across subcells. So, I
            % can just ignore any averaging or anything like that and go
            % directly into unstacking every cell's struct into an array
            for cond = 1:numConds
                for rep = 1:size(responses_cellSep{c}.(condNames{cond}), 3)
                    % I do need to check for NaNs though before stacking
                    if ~isnan(sum(responses_cellSep{c}.(condNames{cond})(:, :, rep), ...
                            'all'))
                        stackedArrays{c} = [stackedArrays{c}; ...
                            responses_cellSep{c}.(condNames{cond})(:, :, rep)];
                    end
                end
            end
        end
